...
class r_rule{
	...
	inline friend size_t hash_value(r_rule const &);
...
class h_rule: public b_rule{
	...
	inline h_rule(const addr_5tup&, size_t (&)[4]);
	
	inline size_t cal_rela(const std::vector<p_rule> &);
...
inline size_t hash_value(r_rule const & rr){  // hash the detailed range value for the value;
	size_t seed = 0;
	for (size_t i = 0; i < 4; ++i){
...
inline h_rule::h_rule(const addr_5tup & center, size_t (& scope)[4]){
...
inline size_t h_rule::cal_rela(const vector<p_rule> & rList){
