...
using std::string; using std::vector; using std::pair; using std::list; using std::set;
using std::cout; using std::endl;
using std::ifstream; using std::ofstream; using std::stringstream;
using std::thread; 
using std::atomic_uint; using std::atomic_bool; using std::mutex;
using boost::unordered_map; using boost::unordered_set;
namespace fs = boost::filesystem;
namespace io = boost::iostreams;

typedef boost::unordered_map<addr_5tup, size_t>::iterator Map_iter;
...
void tracer::set_para(string loc_para_str){
	...
	for (string str; getline(ff, str); ){
		...
		if (!temp[0].compare("hot_no")){
			hotspot_no = boost::lexical_cast<size_t>(temp[1]);
		}
		...
		if (!temp[0].compare("hr_thres")){
			hot_rule_thres = boost::lexical_cast<size_t>(temp[1]);
		}
		if (!temp[0].compare("h_candi_no")){
			hot_candi_no = boost::lexical_cast<size_t>(temp[1]);
		}
		...
		if (!temp[0].compare("h_scope")){
			...
			for (size_t i = 0; i < 4; i++){
				scope[i] = boost::lexical_cast<size_t>(temp1[i]);
			}
		}
	}
}
...
/* count_proc
 * 
 * output: size_t: the count no. of processors, helps determine thread no.
 */
size_t count_proc(){
...
void tracer::hotspot_prob_b(string bFile, string sav_str){
	...
	for (string str; getline(in, str); ){
		vector<string> temp;
		boost::split(temp, str, boost::is_any_of("\t"));
		if (boost::lexical_cast<size_t>(temp.back()) > hot_rule_thres){
...
void tracer::flow_pruneGen_mp( unordered_set<addr_5tup> & flowInfo, fs::path gen_trace_dir) const{
	...
	boost::unordered_map<addr_5tup, pair<size_t, addr_5tup> > pruned_map;
	...
	for(size_t file_id = 0; file_id < to_proc_files.size(); ++file_id){
...
void tracer::f_pg_st(fs::path ref_file, size_t id, string gen_trace_dir, boost::unordered_map<addr_5tup, pair<size_t, addr_5tup> > * map_ptr) const{
...
void tracer::packet_count_mp(string real_trace_dir, string packet_count_file){
	...
	fs::directory_iterator end;
	boost::unordered_map<addr_5tup, size_t> packet_count_map;
	size_t cpu_count = count_proc();
	mutex mtx;
	atomic_uint thr_n(0);
 ...
 * 	  unordered_map<addr_5tup, size_t> * pc_map_ptr: merge the results of counting
 * 	  atmoic_bool * reachend_ptr: indicate the termination of trace gen
 * 
 * function brief
 * this is the thread function that produce packet counts
 */
void tracer::p_count_st(const fs::path gz_file_ptr, atomic_uint * thr_n_ptr, mutex * mtx, boost::unordered_map<addr_5tup, size_t> * pc_map_ptr, atomic_bool * reachend_ptr){
...
void tracer::printTestTrace(string tracefile){
	...
	set<size_t> hitrule_rec;
	for (size_t i = 0; i<flow_rec_vec.size(); i++){
		stringstream ss;
		...
		for (boost::unordered_set<addr_5tup>::iterator iter=flow_rec_vec[i].begin(); iter != flow_rec_vec[i].end(); iter++){
			size_t src = distance(header_rec.begin(), header_rec.find(iter->addrs[0]));
			size_t dst = distance(header_rec.begin(), header_rec.find(iter->addrs[1]));
			ff<<src<<"\t"<<dst<<endl;
			for (size_t j = 0; j < rList->list.size(); j++){
...
void tracer::pruning_trace(string real_trace_dir, string packet_count_file, string gen_trace_file){
	...
	vector<pair<addr_5tup, size_t> > header_shuffler;
	boost::unordered_map<addr_5tup, size_t> pruned_map;

	const size_t noise_thres = 10;
	const size_t bulk_thres = 0.1*duration*data_rate;
	
	size_t total_packet_count = 0;
	for ( string str; getline(pack_count, str); ){
		vector<string> temp;
		boost::split(temp, str, boost::is_any_of("\t"));
		size_t count = boost::lexical_cast<size_t>(temp[1]);
	...
	size_t trial = 0;
	size_t MaxPacket = duration*data_rate;
	size_t flowID = 0;
...
void tracer::gen_random_trace(string ref_trace, string gen_trace){
	...
	for (string str; getline(ref_trace_stream, str); ){
		vector<string> temp;
		boost::split(temp, str, boost::is_any_of("\t"));
		size_t id = boost::lexical_cast<size_t>(temp[1]);
...
/*
void tracer::set_para( double d_rate, double dur, size_t f_no){
	data_rate = d_rate;
	duration = dur;
	terminT = duration + offset;
	flow_no = f_no;
...
*/
void tracer::p_trace_st(fs::path gz_file_ptr, string gen_trace_str, atomic_uint * thr_n_ptr, boost::unordered_map<addr_5tup, size_t> * pc_map_ptr, atomic_bool * reachend){
	cout<<"Processing:"<<gz_file_ptr.c_str()<<endl;
...
void tracer::gen_local_trace(string ref_trace_dir, string gen_trace, string hotspot_candi_file){
	...
	boost::unordered_map<size_t, addr_5tup> checkform;
	boost::unordered_set<addr_5tup> u_headers;
