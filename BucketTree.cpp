...
typedef vector<size_t>::iterator Iter_id;
...
bucket::bucket(const bucket & bk) : b_rule(bk) {
	...
	related_rules = vector<size_t>();
}

bool bucket::split( const size_t (& dim)[2]){
	if ((~(addrs[dim[0]].mask) == 0) || (~(addrs[dim[1]].mask) == 0)) // check availability
		return false;
	
	size_t mask_old;
	size_t mask_new;
	for (size_t i = 0; i < 4; i++){ // four sons
		bucket * temp = new bucket(*this);
		for (size_t j = 0; j < 4; j++){ // four fields
			if (j == dim[0]){
...
bucket_tree::bucket_tree(rule_list & rL, size_t thr, double pa_perc){
	thres_hard = thr;
	thres_soft = thr*2;
	rList = &rL;
...
void bucket_tree::pre_alloc(){
	vector<size_t> rela_buck_count(rList->list.size(), 0);
	INOallocDet(root, rela_buck_count);

	for (size_t i = 0; i< pa_rule_no; i++){
		size_t count_m = 0;
		size_t idx;
		for (size_t i = 0; i < rela_buck_count.size(); i++){
...
void bucket_tree::INOallocDet (bucket * bk, vector<size_t> & rela_buck_count) const{
	for (Iter_id iter = bk->related_rules.begin(); iter != bk->related_rules.end(); iter++){
