...
class lru_cache{
	...
	public:
		inline lru_cache();
		inline lru_cache(const size_t &, double, double = 0.006);
...
class lru_cache_cab{
	...
	public:
		...
		inline lru_cache_cab(const size_t &, const double &, const double & = 0.006);
		
		inline bool ins_rec(const bucket *, const double &, bool = false);
		inline void fetch_data();
};

class lru_cache_cdr: public lru_cache<size_t>{
	protected:
		size_t req_gen;
		size_t delay_rec[21];
		size_t reuse_count;
	...
	public:
		...
		inline lru_cache_cdr(const size_t &, double, double = 0.006);
		
		inline bool ins_rec(const size_t &, const double &, const vector<size_t> &, bool = false);
...
inline lru_cache_cdr::lru_cache_cdr(const size_t & c, double time, double rt):lru_cache(c,time,rt), req_gen(0){
	reuse_count = 0;
	for (size_t i = 0; i<21; ++i)
		delay_rec[i] = 0;
}

inline bool lru_cache_cdr::ins_rec(const size_t & rule_id, const double & curT, const vector <size_t> & dep_ids, bool newFlow){
