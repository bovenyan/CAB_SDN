...
f_node::f_node(size_t dim){
	if (dim < 2){
		intervals.insert(intervals.end(), range_addr(0, ~size_t(0)));
	}
	else{
		intervals.insert(intervals.end(), range_addr(0, (~size_t(0))>>16));
	}
}
...
bool f_node::insert(const r_rule & rule, size_t dim){
	if ( dim == 4 )
		return false;
